...
Version: 2.1 - Added prompt token scoring and code review fixes (2025-12-05)
"""
...
MODEL_CACHE_DIR = (
    Path(VOLUME_PATH) / "models" / "huggingface"
...
class UnifiedProbeService:
    ...
    def load_model(self):
        ...
        logger.info(f"Loading vLLM 70B model: {self.model_name}")
        ...
        if model_cache_path.exists():
            logger.info(f"✓ Loading from cached model at {model_cache_path}")
            model_to_load = str(model_cache_path)
            download_dir = None
        else:
            logger.info(f"⚠ Model not cached, will download from HuggingFace")
            logger.info(
                f"  Run 'modal run unified_probe_service_70b.py::download_model_to_volume' to cache the model"
            )
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_to_load, token=os.environ.get("HF_TOKEN")
        )
    ...
    def _generate_with_probe_impl(
        ...
        try:
            ...
            if len(token_scores) != len(generated_ids):
                raise ValueError(
                    f"Probe hook captured {len(token_scores)} scores but expected {len(generated_ids)} generated tokens. "
        ...
        try:
            ...
            for probe_name, probe_path in probe_paths.items():
                ...
                if "error" in probe_result:
                    return {
                        "error": f"Probe {probe_name} failed: {probe_result['error']}"
                    }
...
def health_check():
    ...
    if probes_dir.exists():
        ...
        return {
            "status": "healthy",
            "service": "unified-probe-service-70b",
            "probes": probe_files,
        }
    else:
