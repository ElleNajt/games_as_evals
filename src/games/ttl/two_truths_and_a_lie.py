...
def generate_two_truths_and_a_lie(
    ...
    prompt = f"""Generate two truths and a lie about {topic}. Put the lie in a random position (not always last).
...
def calculate_statement_scores(
    tokens: List[str], probs: List[float], entropies: List[float] = None, num_statements: int = 3
    ...
    """
    # Find positions of statement number tokens (1, 2, 3, 4, 5, etc.)
    ...
    # Use the actual number of boundaries found
        ...
        if stmt_num - 1 < len(boundaries):
            ...
            if entropies and len(entropies) > start_idx:
                ...
                if statement_entropies:
                    ...
                    avg_entropies[stmt_num] = None
                    sum_entropies[stmt_num] = None
                    max_entropies[stmt_num] = None
            else:
                avg_entropies[stmt_num] = None
                sum_entropies[stmt_num] = None
                max_entropies[stmt_num] = None
...
def ask_for_lie(
    ...
    # Generate the list of numbers dynamically
    numbers_list = ", ".join(str(i) for i in range(1, num_statements + 1))
    if num_statements == 1:
        stmt_word = "statement"
    else:
        stmt_word = "statements"

    prompt = f"""Here are {num_statements} {stmt_word}:

{statements_text}

Which statement is the lie? Respond with ONLY the number ({numbers_list})."""
