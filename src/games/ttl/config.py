...
class TTLPlayerConfig(PlayerConfig):
    ...
    def __post_init__(self):
        if self.role not in ["deceiver", "auditor"]:
            raise ValueError(f"Invalid role: {self.role}. Must be 'deceiver' or 'auditor'")
...
class TTLConfig(GameConfig):
    ...
    """
    deceiver_config: Optional[TTLPlayerConfig] = None  # Required - set in __post_init__ if None
    auditor_config: Optional[TTLPlayerConfig] = None  # Optional - can use Claude for ground truth only
    ...
    def __post_init__(self):
        ...
        if self.deceiver_config.role != "deceiver":
            raise ValueError(f"deceiver_config must have role='deceiver', got '{self.deceiver_config.role}'")
        
        # Validate auditor role if provided
        if self.auditor_config is not None and self.auditor_config.role != "auditor":
            raise ValueError(f"auditor_config must have role='auditor', got '{self.auditor_config.role}'")
