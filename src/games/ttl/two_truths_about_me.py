...
def run_experiment(
    ...
    if avg_entropies:
        ...
        # Still save avg_entropies even if empty (for debugging)
        result_dict.update({
            "avg_entropies": {str(k): v for k, v in avg_entropies.items()} if avg_entropies else {},
            "sum_entropies": {str(k): v for k, v in sum_entropies.items()} if sum_entropies else {},
            "max_entropies": {str(k): v for k, v in max_entropies.items()} if max_entropies else {},
        })
...
def generate_random_facts(
    service, probe_id: str = "llama3_1_8b_lora_lambda_kl=0.5", repo_id: Optional[str] = None
) -> List[str]:
