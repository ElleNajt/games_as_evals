...
class GameCoordinator:
    def __init__(
        self,
        config: Optional[WerewolfConfig] = None,
        experiment_name: str = "game",
        game_id: Optional[int] = None,
    ):
        """
        Initialize game coordinator with unified config and logging system.

        Args:
            config: WerewolfConfig instance or None (will use defaults)
            experiment_name: Base experiment name (gets appended with git/config hashes)
        ...
        if game_id is None:
            ...
            # Recreate logger with proper game_id
            self.logger = ResultsLogger(
                config=self.config,
                game_name="werewolf",
                experiment_base=experiment_name,
        ...
        for player_config in self.config.players:
            if "Werewolf" in player_config.name:
                role = Role.WEREWOLF
            elif "Seer" in player_config.name:
                role = Role.SEER
            else:
                role = Role.VILLAGER
        ...
        for name, role in zip(player_names, roles):
            ...
            if player_config:
                # Create backend for this player
