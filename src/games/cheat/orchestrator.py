...
"""
    ...
    def _parse_play_response(
        ...
        try:
            ...
            # Parse claimed ranks from CLAIM line (for determining count and what was claimed)
            ...
            # CRITICAL FIX: Only play as many cards as claimed, not all cards listed in ACTUAL_CARDS
            # This prevents the bug where player plays 4 cards but claims 2
            cards_to_play = []
            hand_copy = player_state.hand.copy()

            # Only take the first claimed_count cards from actual_ranks
            ranks_to_use = actual_ranks[:claimed_count]
