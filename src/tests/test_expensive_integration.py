...
class TestBackendIntegration8B:
    ...
    def test_8b_single_probe_hallucination(self):
        ...
        backend = create_backend("modal", probe="hallucination_8b", top_k_logits=10)
...
class TestTTLGame8B:
    ...
    def test_ttl_8b_both_probes(self):
        ...
        # Verify config
        assert set(config.deceiver_config.probes) == {"deception_8b", "hallucination_8b"}
...
@pytest.mark.skip(reason="70B requires 4x H100 GPUs - only run when 70B service is deployed")
class TestBackendIntegration70B:
    ...
    def test_70b_single_probe_deception(self):
        ...
        backend = create_backend("modal", probe="deception_70b", top_k_logits=10)
    ...
    def test_70b_both_probes(self):
        ...
        backend = create_backend(
            ...
            top_k_logits=exp_config.top_k_logits
        )
...
class TestTTLGame70B:
    ...
    def test_ttl_70b_both_probes(self):
        ...
        # Verify config
        assert set(config.deceiver_config.probes) == {"deception_70b", "hallucination_70b"}
        assert set(config.auditor_config.probes) == {"deception_70b", "hallucination_70b"}
        
        # Run one round
        results = run_game_round(
            config=config,
            facts=None,
            experiment_name="test_ttl_70b_both",
            round_id=1
        )
...
if __name__ == "__main__":
    ...
    print("  pytest src/tests/test_expensive_integration.py -m expensive --run-all -v -s")
