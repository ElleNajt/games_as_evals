...
def get_avg_probe_scores(game: Dict[str, Any]) -> Dict[str, float]:
    """Calculate average probe scores across all probes for each statement position.

    Args:
        game: Game result dict with deceiver_generation.probe_scores

    Returns:
        Dict mapping position (as string) to average score across probes
    """
    ...
    for probe_name, scores in probe_scores.items():
        positions.update(scores.keys())

    # Calculate average for each position
    avg_scores = {}
    for pos in positions:
        scores_for_pos = [
            probe_scores[probe_name][pos]
            for probe_name in probe_scores
            if pos in probe_scores[probe_name]
        ]
        avg_scores[pos] = (
...
def plot_auditor_accuracy_over_time(data: Dict[str, Any], output_dir: Path) -> Path:
    ...
    without_probe_correct = [
        1 if g["auditor_correct_without_probe"] else 0 for g in games
    ]

    # Calculate cumulative accuracy
    with_probe_cumulative = np.cumsum(with_probe_correct) / np.arange(1, len(games) + 1)
    without_probe_cumulative = np.cumsum(without_probe_correct) / np.arange(
        1, len(games) + 1
    )
    ...
    # Per-round accuracy
    ax1.plot(
        rounds, with_probe_correct, "o-", label="With Probe", color="blue", linewidth=2
    )
    ax1.plot(
        rounds,
        without_probe_correct,
...
def plot_lie_position_analysis(data: Dict[str, Any], output_dir: Path) -> Path:
    ...
    accuracy_by_position = {
        pos: (
            correct_by_position[pos] / total_by_position[pos]
...
def plot_probe_following_behavior(data: Dict[str, Any], output_dir: Path) -> Path:
    ...
    followed_and_correct = sum(
        1 for i in range(len(games)) if followed_probe[i] and probe_was_correct[i]
    )
    followed_and_wrong = sum(
        1 for i in range(len(games)) if followed_probe[i] and not probe_was_correct[i]
    )
    not_followed_and_correct = sum(
        1 for i in range(len(games)) if not followed_probe[i] and probe_was_correct[i]
    )
    not_followed_and_wrong = sum(
        1
        for i in range(len(games))
        if not followed_probe[i] and not probe_was_correct[i]
    )
    ...
    values = [
        followed_and_correct,
        followed_and_wrong,
        not_followed_and_correct,
        not_followed_and_wrong,
    ]
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\nExample:")
        print(
            "  python -m src.visualization.ttl_batch_plots results/ttl/ttl_8b_both_probes_xxx/"
        )
        sys.exit(1)
