...
def run_cheat_game(
    config: CheatConfig, round_id: int, experiment_name: str, **kwargs
) -> Dict[str, Any]:
...
def extract_cheat_stats(game_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "challenge_stats" in game_result:
        ...
        for player, player_stats in challenge_stats.items():
            stats[f"{player}_total_challenges"] = player_stats.get(
                "total_challenges", 0
            )
            stats[f"{player}_correct_challenges"] = player_stats.get(
