...
class TestClaudeBackend:
    ...
    def test_generate_success(self, mock_run):
        ...
        mock_run.return_value = Mock(returncode=0, stdout="This is a test response.")
    ...
    def test_generate_removes_warnings(self, mock_run):
        ...
        mock_run.return_value = Mock(
            returncode=0, stdout="path was not found\nThis is the actual response."
        )
...
class TestModalBackend:
    ...
    def test_init_with_single_probe(self):
        backend = ModalBackend(probes=["deception_8b"])
    ...
    def test_init_with_logits(self):
        backend = ModalBackend(probes=["deception_8b"], top_k_logits=10)
    ...
    def test_init_with_explicit_app_name(self):
        backend = ModalBackend(probes=["deception_8b"], modal_app_name="custom-app")
    ...
    def test_supports_logits_disabled(self):
        backend = ModalBackend(probes=["deception_8b"], top_k_logits=0)
        assert backend.supports_logits == False
...
class TestGenerationResult:
    ...
    def test_full_result(self):
        probe_data = ProbeScoreData(
            aggregate_score=0.75, token_scores=[0.7, 0.8], metadata={"test": "value"}
        )
        probe_scores = ProbeScores(scores={"test_probe": probe_data})
...
class TestProbeScores:
    ...
    def test_minimal_scores(self):
        probe_data = ProbeScoreData(aggregate_score=0.5, token_scores=[0.4, 0.6])
        scores = ProbeScores(scores={"test_probe": probe_data})
